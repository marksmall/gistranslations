...
pipeline {
  ...
  environment {
    ...
    DOCKER_REPOSITORY="339570402237.dkr.ecr.eu-west-1.amazonaws.com/company/thermcert/dashboard/django"
  }
  ...
  stages{
    ...
    stage("Prepare environment") {
      steps {
        ...
        sh """docker run -h ${DJANGO_SETTING_DATABASE_HOST} --network ${DOCKER_NETWORK} \
            ...
            -e POSTGRES_USER=thermcert_user -e POSTGRES_PASS=thermcert_password \
            -e ALLOW_IP_RANGE=0.0.0.0/0 -e POSTGRES_DBNAME=thermcert_db kartoza/postgis:9.6-2.4"""
      }
    }
    ...
    stage("Backend: Tests") {
      ...
      steps {
        dir("server") {
          sh "mkdir -p /var/log/thermcert/"
          sh "touch /var/log/thermcert/django.log"
    ...
    stage("Frontend: E2E Cypress Tests") {
      ...
      steps {
        withCredentials([file(credentialsId: 'company.thermcert.env.local', variable: 'THERMCERT_ENV_LOCAL_FILE')]) {
          sh "rm -f .env.local || true"
          sh "cp \$THERMCERT_ENV_LOCAL_FILE .env.local"
        }
    ...
    stage("Deploy: Testing") {
      ...
      steps{
        build job: '/THERMCERT/dashboard/deploy-to-swarm', propagate: false, parameters: [
  ...
  post {
    ...
    success {
      slackSend channel: '#thermcert-code', color: 'good', message: "ThermCERT: Successful build of ${BRANCH_NAME}!"
    }
    failure {
      slackSend channel: '#thermcert-code', color: 'warning', message: "ThermCERT: Unsuccessful build of ${BRANCH_NAME} :finnadie: ${JOB_URL}"
    }
  }
}
